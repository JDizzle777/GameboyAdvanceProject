...
void GameState::SetupStage_Level1() {
   ...
   for(int i = 0; i < MAX_ENEMIES; i++){
      ...
      InitObject(kObj_Monster[i], ksprites_Monster-2);
...
void GameState::Update_Level1() {
   ...
   int width, height, x, y;
	...
	if (CheckKeyPress(KEY_A) && !_isShooting) {
      ...
      ShowObject(kObj_playerLaser, true);
   }
   ...
   if(frame == 20 || frame == 40 || frame == 60){
      for(int i = 0; i < numAlien; i++){
         if(_xPlayer < _xAlien[i]){
            ...
            if((_yPlayer <= (_yAlien[i]+5) || _yPlayer >= (_yAlien[i]-5)) && !_has_projectile[i]){
   ...
   if(_isShooting){
      ...
      // check for boundaries
      if(playerLaserX < 0 || playerLaserX > 220){
         _isShooting = false;
         ShowObject(kObj_playerLaser, false);
      }
      else{
   ...
   for(int i = 0; i < numMonster; i++){
      ...
      // check background
      if ((_yMonster[i] < (_yGroundLevel-1)) && (BgMapData[index] == 0x0000 || BgMapData[index] == 0x0004 || BgMapData[index] == 0x0005)) {
         // Apply gravity by reducing upward velocity.
         monsterYVelocity[i]--;
      } 
   ...
   for(int i = 0; i < numAlien; i++){
      ...
      if(_isShooting){
         if(CollidePixel(kObj_playerLaser, kObj_Alien[i])){
            if(alienHP[i] == 1){
   ...
   for(int i = 0; i < numMonster; i++){
      ...
      if(_isShooting){
         if(CollidePixel(kObj_playerLaser, kObj_Monster[i])){
            if(monsterHP[i] == 1){
               MoveObjectTo(kObj_playerLaser, -20, -35);
               MoveObjectTo(kObj_Monster[i], -20, -35);
               ShowObject(kObj_Monster[i], false);
